...
TEST(test_rgb2yuv_white) {
    ...
    color_yuv_t yuv;
    rgb2yuv(&rgb, &yuv);
    // Y should be ~1.0 for white
    ASSERT_FLOAT_EQ(yuv.y, 1.0f, 0.01f);
    // U and V should be near 0 for achromatic
...
TEST(test_rgb2yuv_red) {
    ...
    ASSERT_TRUE(yuv.v > 0.0f); // red has positive V
}
...
TEST(test_rgb2yuv_blue) {
    ...
    ASSERT_TRUE(yuv.u > 0.0f); // blue has positive U
}
...
TEST(test_init_free_hash_colors) {
    // Should not crash - init and free are idempotent
    // (hash_colors was already initialized in main, so free and re-init)
    free_hash_colors();
    init_hash_colors();
...
TEST(test_convert_color_exact_palette) {
    // Pure red (0xFF0000) is in the palette, should map to itself
...
TEST(test_convert_color_near_white) {
    // Near-white should map to white (0xFFFFFF)
...
TEST(test_convert_color_near_black) {
    // Very dark should map to black or near-black palette entry
    color_t col = {1, 1, 1, 255};
    color_t out = {0, 0, 0, 255};
    convert_color(&col, &out);
    // Should map to one of the dark grays (0x080808 or 0x000000)
    ASSERT_TRUE(out.r <= 8);
    ASSERT_TRUE(out.g <= 8);
    ASSERT_TRUE(out.b <= 8);
}
...
TEST(test_terminal_columns_nonzero) {
    uint32_t cols = terminal_columns();
    ASSERT_TRUE(cols > 0);
}

TEST(test_terminal_rows_nonzero) {
...
TEST(test_img_create_dimensions) {
    ...
    // img_create doesn't set width/height, just allocates pixels
...
TEST(test_img_resize_downscale) {
    ...
    // After resize, should still be red (area average of all-red = red)
    for (uint32_t i = 0; i < 16; i++) {
...
TEST(test_img_convert_colors) {
    ...
    // Set to a color that's in the palette (pure green 0x008000)
    ...
    // After conversion, colors should be valid palette entries
    for (uint32_t i = 0; i < 4; i++) {
...
TEST(test_img_free) {
    ...
    img.delays = NULL;
    img_free(&img);
    // If we get here without crash, free works
}
...
TEST(test_img_free_with_delays) {
    ...
    img.delays[1] = 200;
    img_free(&img);
    // If we get here without crash, free works with delays
}
...
// ============================================================================
// pixelToInt tests (static in catimg.c, re-implemented here for testing)
// ============================================================================

static uint32_t test_pixelToInt(const color_t *pixel) {
    if (pixel->a == 0)
        return 0xffff;
...
TEST(test_pixelToInt_transparent) {
    color_t pixel = {100, 100, 100, 0};
    ASSERT_EQ(test_pixelToInt(&pixel), 0xffff);
}

TEST(test_pixelToInt_black) {
    color_t pixel = {0, 0, 0, 255};
    ASSERT_EQ(test_pixelToInt(&pixel), 232); // grayscale ramp starts at 232
}

TEST(test_pixelToInt_white) {
    color_t pixel = {255, 255, 255, 255};
    ASSERT_EQ(test_pixelToInt(&pixel), 255); // 232 + 23
}

TEST(test_pixelToInt_gray) {
    color_t pixel = {128, 128, 128, 255};
    uint32_t result = test_pixelToInt(&pixel);
    ASSERT_TRUE(result >= 232 && result <= 255);
}

TEST(test_pixelToInt_red) {
    color_t pixel = {255, 0, 0, 255};
    uint32_t result = test_pixelToInt(&pixel);
    ASSERT_EQ(result, 16 + 5*36); // 196
}

TEST(test_pixelToInt_green) {
    color_t pixel = {0, 255, 0, 255};
    uint32_t result = test_pixelToInt(&pixel);
    ASSERT_EQ(result, 16 + 5*6); // 46
}

TEST(test_pixelToInt_blue) {
    color_t pixel = {0, 0, 255, 255};
    uint32_t result = test_pixelToInt(&pixel);
    ASSERT_EQ(result, 16 + 5); // 21
}
...
int main(void) {
    ...
    RUN_TEST(test_terminal_columns_nonzero);
    RUN_TEST(test_terminal_rows_nonzero);
    ...
    printf("\n=== pixelToInt tests ===\n");
    RUN_TEST(test_pixelToInt_transparent);
    RUN_TEST(test_pixelToInt_black);
    RUN_TEST(test_pixelToInt_white);
    RUN_TEST(test_pixelToInt_gray);
    RUN_TEST(test_pixelToInt_red);
    RUN_TEST(test_pixelToInt_green);
    RUN_TEST(test_pixelToInt_blue);
