...
int main(int argc, char *argv[])
{
    ...
    init_hash_colors();
    char *files[argc]; // seems a sensible initialization
    char *num;
    int c, i = 0, j = 0;
    opterr = 0;
    ...
    for (; optind < argc; optind++) {
        ...
        if (strncmp(files[i], "-", 1) != 0 &&  access(files[i], F_OK) == -1) {
            // if not STDIN and file can't be accessed
            printf(ERR_FILE_NOT_FOUND, files[i]);
            exit(1);
        } else if (strncmp(files[i], "-", 1) == 0 && (i > 0 || (optind+1) < argc)) {
            // if STDIN and either there were already other files passed or
            // there are more to come -> abort
            printf(ERR_FILE_STDIN_NOT_ALLOWED);
            exit(1);
        }
        i++;
    }
